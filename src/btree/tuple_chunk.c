...
void
tuple_chunk_compact(BTreeChunkDesc *desc)
{
	uint16		itemsSize;
	Pointer		batchDst,
				batchSrc;
	...
	Assert(desc->chunkType == BTreeChunkLeaf);

	if (desc->chunkItemsCount == 0)
		return;

	/* Detect deleted items and delete them from the items array */
	for (OffsetNumber itemOffset = 0; itemOffset < desc->chunkItemsCount; itemOffset++)
	{
		BTreeLeafTuphdr *tupleHeader;
		OTuple		tuple;

		read_tuple_chunk_leaf_item(desc, itemOffset, &tupleHeader, &tuple);
		if (tupleHeader->deleted == BTreeLeafTupleNonDeleted)
			desc->chunkItems->items[lastOffset++] = desc->chunkItems->items[itemOffset];
	}
	desc->chunkItemsCount = lastOffset;

	/* All items were deleted, just exit */
	if (desc->chunkItemsCount == 0)
	{
		desc->chunkDataSize = 0;
		return;
	}

	itemsSize = MAXALIGN(sizeof(LocationIndex) * desc->chunkItemsCount);
	batchDst = desc->chunkData + itemsSize;
	...
	for (OffsetNumber itemOffset = 0; itemOffset < desc->chunkItemsCount; itemOffset++)
	{
		 ...
		 */
		if (itemOffset > 0 && get_tuple_chunk_item(desc, itemOffset) >
			(batchDst + batchSize))
		{
			batchSrc = get_tuple_chunk_item(desc, batchOffset);
		...
		tuple = read_tuple_chunk_tuple(desc, itemOffset);
		batchSize += get_tuple_chunk_header_size(desc->chunkType) +
			MAXALIGN(get_tuple_chunk_tuple_size(desc->treeDesc,
												desc->chunkType, tuple));
	}
	...
	if (batchDst != batchSrc)
	{
		...
		/* Adjust the items array */
		for (OffsetNumber i = batchOffset; i < desc->chunkItemsCount; i++)
			desc->chunkItems->items[i] -= dataShift;
				...
				void *key, BTreeKeyType keyType, int *result)
{
	...
	keyType2 = desc->chunkType == BTreeChunkLeaf ? BTreeKeyLeafTuple : BTreeKeyNonLeafKey;
	tuple2 = read_tuple_chunk_tuple(desc, itemOffset);
...
{
	bool		isLeaf = desc->chunkType == BTreeChunkLeaf,
				nextKey;
	...
	while (high > low)
	{
		...
		else
		{
			...
			*itemOffset = mid;
			midTuple = read_tuple_chunk_tuple(desc, *itemOffset);
					   ...
					   Pointer *tupleHeader, OTuple *tuple, bool *needsFree)
{
	...
	if (desc->chunkType == BTreeChunkLeaf)
		read_tuple_chunk_leaf_item(desc, itemOffset,
								   (BTreeLeafTuphdr **) tupleHeader, tuple);
	else
		read_tuple_chunk_internal_item(desc, itemOffset,
...
init_tuple_chunk(BTreeChunkDesc *desc, Page page, OffsetNumber chunkOffset)
{
	...
	desc->chunkItems = (BTreePageChunk *) (page + chunkLocation);
					...
					uint16 itemSize)
{
	...
	if (desc->chunkItemsCount > 0)
		firstItemPtr = get_tuple_chunk_item(desc, 0);
	else
	...
	Assert(desc->chunkDataSize + dataShift <= ORIOLEDB_BLCKSZ);
