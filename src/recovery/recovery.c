...
{
	int			num = *((int *) arg);
	const Pointer pa = (const Pointer) a;
	const Pointer pb = (const Pointer) b;
	const RecoveryXidState *l = (RecoveryXidState *) (pa - offsetof(RecoveryXidState, retain_undo_ph_nodes) - sizeof(pairingheap_node) * num);
	const RecoveryXidState *r = (RecoveryXidState *) (pb - offsetof(RecoveryXidState, retain_undo_ph_nodes) - sizeof(pairingheap_node) * num);
								 ...
								 XLogRecPtr recoveryPtr)
{
	...
	if (pairingheap_is_empty(retain_undo_queues[undoType]))
		return false;
	ptr = (Pointer) pairingheap_first(retain_undo_queues[undoType]);
	state = (RecoveryXidState *) (ptr - offsetof(RecoveryXidState, retain_undo_ph_nodes) - sizeof(pairingheap_node) * (int) undoType);
